...
def interpolate_manifold(text: torch.Tensor, beta: float=0.5) -> torch.Tensor:
    B, _ = text.shape

    p1 = torch.randperm(B, device=text.device)
    p2 = torch.randperm(B, device=text.device)

    same = p1 == p2
    if same.any():
        p2[same] = p2[same].roll(1)

    a = text[p1]                         # [B, 1024]
    b = text[p2]                         # [B, 1024]

    interpolation = beta*a + (1-beta)*b  # [B, 1024]
...
class Trainer:
    def __init__(
        ...
        self.inter = interpolation_weight
    ...
    def train(self) -> None:
        ...
        for epoch in range(0, epochs):
            ...
            for img, wrong_img, text, _, _ in dataloader:   # (match img, match text, match text (str), match label)
                batch = img.size(0)                                     # N := batch

                img = img.to(device)                                    # [N, 3, 64, 64]
                wrong_img = wrong_img.to(device)                        # [N, 3, 64, 64]
                text = text.to(device)                                  # [N, 1024]
                ...
                wrong_score = discriminator(wrong_img, text)            # wrong image, right text
                ...
                text_int = interpolate_manifold(text)

                # =============================================================
                # ==================== Update generator =======================
                # =============================================================
                noise_1 = torch.randn(size=(batch, nz, 1, 1)).to(device) # [N, 100]
                noise_2 = torch.randn(size=(batch, nz, 1, 1)).to(device) # [N, 100]

                fake_img_1 = generator(noise_1, text)                    # [N, 3, 64, 64]
                fake_img_2 = generator(noise_2, text_int)                # [N, 3, 64, 64]
 
                fake_score_1 = discriminator(fake_img_1, text)           # fake image, right text
                fake_score_2 = discriminator(fake_img_2, text_int)       # fake image, "right text"

                loss_g_1 = criterion_g(fake_score_1)
                loss_g_2 = criterion_g(fake_score_2)

                loss_g = loss_g_1 + int_weight * loss_g_2
            ...
            if epoch % cfg.save_every == 0:
                checkpoint_dcgan(cfg, generator, discriminator, report)
