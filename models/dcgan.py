...
class Generator(nn.Module):
    ...
    def __init__(
        ...
        # [N, 1024] -> [N, 128]
        self.proj_manifold = ProjectManifold(emb_in, emb_out)
        
        # [N, 100 + 128, 1, 1] -> [N, 8*ngf, 4, 4] 
        self.up1 = nn.Sequential(
            nn.ConvTranspose2d(z_dim + emb_out, 8 * ngf, 4, 1, 0, bias=False),
        ...
        # [N, 4*ngf, 8, 8] -> [N, 2*ngf, 8, 8] ... [N, 2*ngf, 8, 8]-> [N, 4*ngf, 8, 8]
        self.res8x8 = ResidualBottleneck8x8(4*ngf, 2*ngf)
...
class Discriminator(nn.Module):
    ...
    def __init__(
        ...
        self.fuse = nn.Sequential(
            ...
            nn.Conv2d(ndf * 8 + emb_out, ndf * 8, 1, 1, 0, bias=False),
            nn.BatchNorm2d(8*ndf),
            nn.LeakyReLU(0.2, True),
