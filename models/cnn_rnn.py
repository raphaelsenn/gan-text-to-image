...
class CNNRNNEncoder(nn.Module):
    ...
    """ 
    def __init__(self, vocab_size: int, cnn_dim: int=512, emb_dim: int=1024):
        super().__init__()

        # NOTE: Here, we use Threshold to match the orignal paper (ReLU would work too!).
        self.cnn = nn.Sequential(
            # [N, vocab_size, 201] -> [N, 384, 66]
            nn.Conv1d(vocab_size, 384, kernel_size=4),
            nn.Threshold(1e-6, 0.0, inplace=True),
            nn.MaxPool1d(3, 3),
            ...
            # [N, cnn_dim, 21] -> [N, cnn_dim, 18]
            nn.Conv1d(512, cnn_dim, kernel_size=4),
            nn.Threshold(1e-6, 0.0, inplace=True), 
        )
        # [N, cnn_dim, 18] -> [N, cnn_dim] 
        self.gru = nn.GRU(cnn_dim, cnn_dim, num_layers=1, bias=True, batch_first=True)
        
        # [N, cnn_dim] -> [N, emb_dim]
        self.out = nn.Linear(cnn_dim, emb_dim)
